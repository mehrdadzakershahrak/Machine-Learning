...
def extract_records(lines,author_citation):
    records = []
    title = ''
    authors = ''
    year = ''
...
author_citation = {}
records = extract_records(lines, author_citation)
...
with open('author.csv', 'wb') as f:
    f.write('author, citations')
    for k, v in author_citation.iteritems():
        f.write('\n' + k + ',' + str(v))
    f.close()

with open('output.csv', 'wb') as f:
    f.write('index, arnetid, citations, year, pubvenue, title_fv,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,, abstract_fv')
    j = 0
    for rec in records:
        if j % 50000 == 0:
            print j
        j = j + 1
        f.write('\n' + rec['index'] + ',' + rec['arnetid'] + ',' + rec['citations'] + ',' + rec['year'] + ',"' + rec['pubvenue'] + '"')
