...
def read_all_lines(file_object):
    """Shirazi(Lazy) function (generator) to read a file piece by piece.
...
def extract_records(lines):
    ...
    for l in lines:
        if l[:2] == "#*":
            ...
            record = {'title': title, 'auhtors': authors, 'year': year, 'pubvenue': pubvenue, 'citations': citations,
                      'index': index, 'arnetid': arnetid, 'abstract': abstract}
